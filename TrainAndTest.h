 ...
 */
// it would be cleaner to make these dynamic but for safety reasons we can't have student code dynamically allocating memory
...
#define NO_PREDICTION -1
    ...
    /// function that calls a machine learning algorithm and learns a classifier
    /// returns 1 if successful,  0 if some error condition occurs
    ...
    /// @param numSamples : int - the number of samples (rows)
...
void StoreData(double **trainingSamples, int *trainingLabels, int numSamples, int numFeatures);
    ...
    ///         Starting with an empty working candidate, at each iteration it shoul the single rule that most increases the score
    ///         to test a candidate solution copy it into th     you should call the function ScoreWorkingCandidateOnTrainingSet() to test a candidate solution
...
bool GoalFound(void);
    ...
    /// @brief Function that calculates and stores the score for the working candidate
...
///             Note that score is zero if any of the training set are classified incorrectly,  otherwise it is the number of correctly classified samples in the training set
...
void ScoreWorkingCandidateOnTrainingSet(void);
    ...
    /// @param numFeatures number of features in sample
...
int PredictClassFromRule(rule thisRule, double * sample, int numFeatures );
    ...
    /// @Brief Function to print out the ruleset represented within a candidate solution
    /// precondition is the existence of a variable called workingCandidate that holds the learned rule set
...
void ExtendWorkingCandidateByAddingRule(rule newRule);
    ...
    /// function that initialises the local storage ready to hold the training data
